 ...
 */
@Slf4j
public class ADT_A01_TransformerTest {

  private static final String HL7_OBX_FILE = "ADT_A01_OBXtest.hl7";
  ...
  /**
   * Test of getApplicableMessageType method, of class ADT_A01_Transformer.
   */
  @Test
  public void testGetApplicableMessageType() {
    ADT_A01_Transformer transformer = new ADT_A01_Transformer();
    assertEquals(transformer.getApplicableMessageType(), ADT_A01.class);
  }

  /**
   * Test of transform method, of class ADT_A01_Transformer.
   *
  ...
  public void testTransform() throws IOException, HL7Exception, URISyntaxException {
    ...
    for (Event event : events) {
      ...
      if (event.getType().equals(EventType.OBSERVATIONS_ADD)) {
        ...
        obxCompleted = true;
      } else if (event.getType().equals(EventType.PATIENT_ADMIT)) {
        assertNotNull(event.getData());

        AdmitPatientData a01 = (AdmitPatientData) event.getData();

        assertEquals(a01.getPatient().getInstitutionPatientId(), "97554803");
    ...
    assertTrue(a01Completed, "A01 message was not successfully processed");
